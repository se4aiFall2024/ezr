...
def guess(N, d):
    """Selects N random rows and returns them sorted on Chebyshev distance."""
    some = random.choices(d.rows, k=N)
    cloned_data = d.clone().adds(some)
    distances = [cloned_data.chebyshev(row) for row in cloned_data.rows]
    sorted_rows = sorted(cloned_data.rows, key=lambda row: distances[cloned_data.rows.index(row)])
    return sorted_rows
